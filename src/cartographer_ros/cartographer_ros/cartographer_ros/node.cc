...
bool Node::HandleTrajectoryQuery(
  ...
  std::string txt_filename3 = "/home/lmf/pcd&traj/sel_time_stamp.txt";
  std::ofstream soutf(txt_filename1, std::ios::app);
  std::ofstream aoutf(txt_filename2, std::ios::app);
  std::ofstream toutf(txt_filename3, std::ios::app);
  for (int i = 0; i < response.trajectory.size(); i++){
   ...
   // double z = response.trajectory[i].pose.position.z;
    ...
    aoutf <<std::setprecision(17) <<sec<< "."<<std::setprecision(6) <<nsec << " " << Total_distance << " " << last_x << " " << last_y  << std::endl;

    if(Euro_distance > 0.5){
        std::cout<<"Euro_distance : "<<Euro_distance<<std::endl;
        // sfs <<std::setprecision(17) <<sec << "." <<nsec << " " << Euro_distance << " " << last_x << " " << last_y  << std::endl;
        soutf <<std::setprecision(17) <<sec << "."<<std::setprecision(6) <<nsec << " " << Euro_distance << " " << last_x << " " << last_y  << std::endl;
        toutf <<std::setprecision(17) <<sec << "." <<std::setprecision(6) <<nsec << std::endl;
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      ...
      if (node_options_.publish_tracked_pose) {
        ...
        tracked_pose_publisher_.publish(pose_msg);
         ...
         //zhanglei add
          if(SaveRange){
        //  string timestring = std::to_string(ToRos(trajectory_data.local_slam_data->time).toSec()).substr(0,17);
             string timestring = std::to_string(pose_msg.header.stamp.toSec()).substr(0,13);
        //  std::cout<<"timestring:"<<timestring<<std::endl;
             LOG(INFO) <<"timestring:"<<timestring<<std::endl;
        //  std::cout<<"vTimStamps:"<<vTimeStamps[0]<<std::endl;
        //  std::cout<<"!!!diff_value:"<<(stod(timestring)-stod(vTimeStamps[0]))<<std::endl;
         ...
         for(auto timestamp:vTimeStamps){
          ...
          //  std::cout<<"timestamp:"<<timestamp<<"double_value:"<<stod(timestamp)<<std::endl;
            //  if(timestamp == timestring){
              ////转为double类型进行比较
             if(stod(timestring)==stod(timestamp)){
                 bematched = true;

                cout.setf(ios::fixed,ios::floatfield);  //不使用科学计数法
                // std::cout<<"timestring:"<<timestring<<" "<<"double_value:"<<stod(timestring)<<std::endl;
                LOG(INFO) <<"timestring:"<<timestring<<" "<<"double_value:"<<stod(timestring)<<std::endl;
                cout.setf(ios::fixed,ios::floatfield);  //不使用科学计数法
                // std::cout<<"timestamp:"<<timestamp<<" "<<"double_value:"<<stod(timestamp)<<std::endl;
               LOG(INFO) <<"timestamp:"<<timestamp<<" "<<"double_value:"<<stod(timestamp)<<std::endl;
                // std::cout<<"!!!diff_value:"<<(stod(timestring)-stod(timestamp))<<std::endl;
                LOG(INFO) <<"!!!diff_value:"<<(stod(timestring)-stod(timestamp))<<std::endl;
                //  continue;
                break;
             }
         }
         if(bematched){
              ...
              //  LOG(INFO) << "Saving map to pcd files ...";
               ...
               //  add by lmf
                ...
                // spoutf <<std::setprecision(17) << pose_msg.header.stamp.sec << "."  << pose_msg.header.stamp. nsec<<" " <<pose_msg.pose.position.x << " " << pose_msg.pose.position.y  << std::endl;
                 spoutf <<std::to_string(pose_msg.header.stamp.toSec())<<" " <<pose_msg.pose.position.x << " " << pose_msg.pose.position.y  << std::endl;
                bematched = false;
                // Delay(50);   //延时50ms 
          }
            else{
...
void Node::LoadTimeStampe(const std::string& _filename)
{
    ...
    {
        vTimeStamps.push_back(timestamp.substr(0,13));
    }
